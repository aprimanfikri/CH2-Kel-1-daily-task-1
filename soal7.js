...
const friendsName = (data) => {
  // tempat penampungan hasil
  let result = [];
  // looping data dalam array
  for (let i = 0; i < data.length; i++) {
    // menyimpan apakah pengguna memiliki teman yang memenuhi kondisi
    let valid = false;
    // looping kedua untuk mengecek satu satu data teman
    for (let j = 0; j < data[i].friends.length; j++) {
      // memeriksa apakah nama teman saat ini tidak cocok dengan Irfi atau Adella
      if (
        data[i].friends[j].name === "Irfi" ||
        data[i].friends[j].name === "Adella"
      ) {
        valid = true;
      }
    }
    // jika user tidak memiliki teman yang memenuhi kondisi maka push data kedalam result
    if (!valid) {
      result.push(data[i]);
    }
  }
  // print total user yang tidak memiliki teman dengan nama Irfi atau Adella
  console.log(
    `Jumlah user yang tidak memiliki teman Irfi atau Adella adalah : ${result.length}`
  );
  console.log("==========================================");
  return result;
};
// panggil fungsi friendsName dan print hasilnya di konsol
console.log(friendsName(users));
