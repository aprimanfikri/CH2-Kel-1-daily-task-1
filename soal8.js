...
const companyL = (data) => {
  ...
  for (let i = 0; i < data.length; i++) {
    // memeriksa apakah nama company berawalan dengan L menggunakan charAt untuk mendapatkan karakter pertama dari nama company
    if (data[i].company.charAt(0) === "L") {
      result.push(data[i]);
    }
  }
  // print total user yang memiliki company dengan huruf depan L
  console.log(
    `Jumlah user yang company huruf depannya berawal L adalah: ${result.length}`
  );
  console.log("==========================================");
...
// panggil fungsi companyL dan print hasilnya di konsol
console.log(companyL(users));
